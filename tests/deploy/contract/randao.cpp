 ...
 *
 * 1. RANDAO
 * 的确让随机数难以预测，并且随机数享有和底层共识协议一样的活性，但攻击者可以拖到
 * Reveal（披露）阶段的最后再决定是否揭示
 * 自己的承诺，虽然存在质押损失，但是如果收益更高还是有攻击动机的。
 * 2. 可以使用 VDF
 * 替换掉最后的那个异或运算，将其改变为执行时间必定长于各方随机数披露等待期的操作。如果计算最终结果的时间比随机数
...
CONTRACT Randao : public platon::Contract {
 ...
 public:
  ...
  /**
   * @brief 随机数需求方，想要生成随机数，首先需要新建一轮活动，需要使用
   * NewCampaign 函数 比如当前块高度是 1840602，而我们要在 1840900
   * 这个块需要一个随机数，希望每个参与者提交押金为 20，在目标块 前 200
   * 个块开始提交（即从 1840700 块起， 包含 1840700)， 在目标块前 100
   * 个块结束提交（即 1840800块截止，包含 1840800块）。 在 1840800 之后（不包含
   * 1840800）1840900 块之前（不包含 1840900）  属于 Reveal
   * 阶段，可以使用如下方式调用： NewCampaign(1840700, 20000000000000000000,
   * 200, 100)，并且可以发送作为参与者奖励费用。
   *
  ...
  /**
   * @brief
   * 随机数需求方可以选择不创建一轮活动，而是选择跟随某一轮随机数活动作为自己的随机数，这时可以使用
   * Follow 函数
   * 跟随活动必须是在提交随机数窗口期或之前进行，否则就会失败。以前面例子为例，跟随活动必选在
   * 1840800块之前（包括 1840800）。
  ...
  /**
   * @brief 参与者可以通过提交随机数参与随机数的生成，提交随机数可以调用 Commit
   * 函数
  ...
  /**
   * @brief 披露随机数
   * 在随机数提交阶段结束之后，进入 Reveal
   * 阶段，随机数提交者可以披露自己的随机数，合约验证是否是有效的随机数。
   * 如果有效，将计算到最终的随机数结果中，已前面的例子为例，随机数披露窗口期为：
   * 1840800 到 1840900
   *
   * @param campaign_id 已存在的活动 id
   * @param s 随机数
  ...
  /**
   * @brief 退还奖金
   * 如果本轮随机数生成失败，随机数需求方可以通过 RefundBounty
   * 函数，返还其提交的奖励
   *
   * @param campaign_id 已存在的活动 id
   *
